...
export default function Create(props) {
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        post(route("auth.store-kelola-galery"), {
            onSuccess: () => {
                Swal.fire({
                    title: "Sukses",
                    text: "Berhasil menyimpan 1 galery baru",
                    icon: "success",
                });
                reset();
                setPreview(null);
            ...
            onError: () => {
                Swal.fire({
                    title: "Error",
                    text: "gagal menyimpan 1 galery baru",
                    icon: "error",
                });
            },
        });
    ...
    const updateHandler = (e) => {
        e.preventDefault();
        post(route("auth.update-kelola-galery", galery.slug), {
            onSuccess: () => {
                Swal.fire({
                    title: "Sukses",
                    text: "Berhasil mengupdet 1 berita",
                    icon: "success",
                });
                reset();
                setPreview(null);
    ...
    useEffect(() => {
        if (galery !== undefined) {
            setData({
                ...data,
                judul_berita: galery?.judul_berita,
                kontent: galery?.kontent,
            });
            setPreview("/storage/" + galery?.thumbnail);
        }
    }, []);
    console.log(galery);
            ...
            <div className="py-3 w-full">
                <div className="flex justify-between items-center gap-x-3">
                    ...
                    <button
                        type="submit"
                        onClick={
                            galery == undefined ? submitHandler : updateHandler
                        }
